...
<script>
  // const SimpleSignalClient = require('./simple-signal-actioncable.js')
  import SimpleSignalClient from './simple-signal-actioncable.js'
  ...
  export default {
    ...
    methods: {
      async join() {
        ...
        this.log('join')
        // TODO:
        // Do we need to implement the server within ActionCable? Hopefully not?
        // https://github.com/t-mullen/simple-signal/blob/master/server/src/index.js
        //
        // init SimpleSignalClient from Rails ActionCable consumer:
        this.signalClient = new SimpleSignalClient(consumer)
        let constraints = {
        ...
        this.signalClient.once('discover', (discoveryData) => {
          ...
          async function connectToPeer(peerID) {
            // if (peerID == that.consumer.id) return
            try {
        ...
        this.signalClient.discover({ channel: 'RoomChannel', id: that.roomId })
      },
